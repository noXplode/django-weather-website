...
def GetGeoIPCity(ip):
    g = GeoIP2()
    # geoipdict = {'city': 'Kharkiv', 'continent_code': 'EU', 'continent_name': 'Europe',
...
def getpopulars(days=365):  # gets last 30 picked cities for popular block
    pl = Popularity.objects.filter(pickeddate__gt=timezone.now() - timedelta(days=days))[:30]
    pl = list(set([i.city for i in pl]))
    return pl
...
def index(request):
    ...
    context = {'city_info': view_cities, 'geoipdata': geoipdata,
               'form': form, 'populars': populars}
...
def getYAtranslation(text, direction):  # direction format 'en-ru'
    ...
    prms = {'key': '',  # insert yandex translate api key here
            'text': text,
            'lang': direction, }
